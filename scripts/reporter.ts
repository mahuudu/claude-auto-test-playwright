...
class JsonReporter implements Reporter {
  ...
  onTestEnd(test: TestCase, result: TestResult) {
    ...
    // Normalize to relative forward-slash path (handles Windows backslashes)
    const testFile = path.relative(process.cwd(), test.location.file).replace(/\\/g, '/');

    // Map test file → spec file convention: tests/e2e/auth/foo.spec.ts → docs/specs/auth/foo.spec.md
    const specFile = testFile
      .replace(/^tests\/e2e\//, 'docs/specs/')
    ...
    // Screenshot: luôn lưu path (cả pass lẫn fail), dùng forward slash
    let screenshotPath: string | null = null;
    const attachment = result.attachments.find(a => a.name === 'screenshot' && a.path);
    if (attachment?.path) {
      screenshotPath = path.relative(process.cwd(), attachment.path).replace(/\\/g, '/');
    }
